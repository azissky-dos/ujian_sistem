...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .auth-card { background: white; border-radius: 32px; padding: 40px; width: 100%; max-width: 650px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25); }
        ...
        .alert.info { background: #e0e7ff; color: #4338ca; border-left: 4px solid #4338ca; }
        .checkbox-group { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; max-height: 300px; overflow-y: auto; padding: 12px; border: 1px solid #e2e8f0; border-radius: 12px; background: #f8fafc; }
        .checkbox-item { display: flex; align-items: center; gap: 10px; padding: 8px 12px; background: white; border-radius: 10px; cursor: pointer; transition: all 0.2s; border: 1px solid #e2e8f0; }
        .checkbox-item:hover { background: #e0e7ff; border-color: #6366f1; }
        .checkbox-item input { width: 18px; height: 18px; cursor: pointer; accent-color: #4f46e5; }
        .checkbox-item label { margin: 0; cursor: pointer; font-weight: normal; color: #1e293b; font-size: 13px; flex: 1; }
        .checkbox-item label strong { color: #4f46e5; }
        .select-all { margin-bottom: 12px; padding: 10px 12px; background: #f1f5f9; border-radius: 10px; display: flex; align-items: center; gap: 10px; border: 1px solid #e2e8f0; }
        .select-all input { width: 18px; height: 18px; cursor: pointer; accent-color: #4f46e5; }
        .select-all label { margin: 0; cursor: pointer; font-weight: 600; color: #1e293b; font-size: 14px; }
        ...
        @media (max-width: 640px) { .auth-card { padding: 24px; } .checkbox-group { grid-template-columns: 1fr; } }
    </style>
</head>
<body>
    ...
    <div class="auth-card">
        ...
        <form method="POST" id="registerForm">
            ...
            <div class="form-group">
                ...
                <div id="mk_checklist">
                    <div class="alert info"><i class="fas fa-info-circle"></i> Silakan pilih kelas terlebih dahulu</div>
                </div>
            </div>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(data.length > 0) {
                        ...
                        html = '<div class="alert error"><i class="fas fa-exclamation-triangle"></i> Belum ada mata kuliah di kelas ini.</div>';
                    }
                ...
                error: function() {
                    $('#mk_checklist').html('<div class="alert error"><i class="fas fa-exclamation-circle"></i> Gagal memuat data mata kuliah.</div>');
                }
            });
        } else {
